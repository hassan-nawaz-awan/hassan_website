...
export default function MolecularBackground({
  ...
  variant?: 'auto' | 'masthead';
}) {
  const basin1 =
    variant === 'masthead' ? 'text-accent-dark' : 'text-accent dark:text-accent-dark';
  const basin2 =
    variant === 'masthead' ? 'text-ember-dark' : 'text-ember dark:text-ember-dark';
  const faint =
    variant === 'masthead'
      ? 'text-masthead-ink'
